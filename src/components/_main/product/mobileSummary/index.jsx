...
export default function ProductDetailsSumaryMobile({ ...props }) {
  ...
  const whatsappMessage = encodeURIComponent(
    `I'm interested in your product ${product?.name}.\n\n` +
      ...
      `- Quantity: ${values.quantity}\n` +
      `- Total: ₹${(product?.priceSale || product?.price) * values.quantity}\n` +
      `- Link: ${window?.location.href + `product/${product?.slug}`}`
  );
            ...
            <Typography variant="h4" className="text-price">
              {product?.price <= product?.priceSale ? null : (
                <Box component="span" className="old-price">
                  ₹{!isLoading && isLoaded && product?.price}
                </Box>
              )}
              <Box component="span">
                &nbsp; ₹{!isLoading && isLoaded && product?.priceSale}
              </Box>
            </Typography>
          </Stack>
            ...
            {product?.price > product?.priceSale && (
              <Stack direction="row" alignItems="center" spacing={1}>
                ...
                >
                  ...
                  {!isLoading &&
                    isLoaded &&
                    (product?.price - product?.priceSale).toFixed(2)}
                  {
                    <span>
                      (
                      {(
          ...
          <Stack spacing={2} className="detail-actions-wrapper">
            ...
            >
              ...
              <Button
                ...
                onClick={() =>
                  window.open(
                    `https://wa.me/+919583967497?text=${whatsappMessage}`,
                    "_blank"
                  )
                }
                variant="contained"

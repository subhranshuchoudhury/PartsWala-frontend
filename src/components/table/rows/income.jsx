...
export default function IncomeList({
  ...
  return (
    <TableRow hover key={Math.random()}>
      ...
      <TableCell>
        {isLoading ? <Skeleton variant="text" /> : row?.orders?.length || 0}
      </TableCell>
      <TableCell>
        {isLoading ? <Skeleton variant="text" /> : fCurrency(row.total)}
      </TableCell>
      <TableCell>
        {isLoading ? <Skeleton variant="text" /> : fCurrency(row.totalIncome)}
      </TableCell>
      <TableCell>
        {isLoading ? (
          <Skeleton variant="text" />
        ) : (
          fCurrency(row.totalCommission)
        )}
      </TableCell>
      <TableCell>
        {isLoading ? (
          ...
          {isLoading ? (
            ...
            <Tooltip title="Preview">
              <IconButton
                onClick={() =>
                  router.push(
                    `/${isVendor ? "vendor" : "admin"}/payments/${row._id}`
                  )
                }
              >
                <IoEye />
